...
def deal_report(request):
    if request.method == 'POST':
        ...
        if type == '1':
            Report.objects.filter(report_id=report_id).update(result=2)
        else:
            article_id = Report.objects.get(report_id=report_id).article_id
            article = Article.objects.get(article_id=article_id)
            article.delete()
            Report.objects.filter(report_id=report_id).update(result=1)
        return JsonResponse({'errno': 0, 'msg': '已处理'})
    else:
